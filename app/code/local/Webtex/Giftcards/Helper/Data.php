...
class Webtex_Giftcards_Helper_Data extends Mage_Core_Helper_Data
{
    ...
    public function getImageByOptionTypeId($optionTypeId)
    {

        if (!empty($optionTypeId)) {
            $resource = Mage::getSingleton('core/resource');
            $readConnection = $resource->getConnection('core_read');
            $tableName = $resource->getTableName('mageworx_custom_options_option_type_image');
            $query = "SELECT * FROM " . $tableName . " WHERE option_type_id = " . $optionTypeId . " LIMIT 1";
            $results = $readConnection->fetchAll($query);
            if (!empty($results)) {
                $result = current($results);
                if (isset($result['image_file'])) {
                    return Mage::getBaseUrl('media') . 'customoptions/' . $result['image_file'];
                }
            }
