...
class HusseyCoding_CartRuleFix_Model_SalesRuleValidator extends Mage_SalesRule_Model_Validator
{
    ...
    public function process(Mage_Sales_Model_Quote_Item_Abstract $item)
    {
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            $parentProduct = Mage::getModel('catalog/product_type_grouped')->getParentIdsByChild($product->getId());
            if(!empty($parentProduct)){
                $parentId = current($parentProduct);
            ...
            switch ($rule->getSimpleAction()) {
                ...
                case Mage_SalesRule_Model_Rule::BY_PERCENT_ACTION:
                    $helper = $this->_getBrosolutionsHelper();
                    ...
                    if($helper){
                        $needApplyRule = $helper->checkIfNeedApplyRule($item);
                        if(!$needApplyRule){
                            $rulePercent = 0;
                        }
                        $customerIsMember = $helper->isCustomerMember($rule);
                        $ruleIdCond = $helper->checkRuleIdForMembers($rule->getId());
                        if($ruleIdCond && !$customerIsMember){
                            $rulePercent = 0;
                        }
                    }
    ...
    protected function _getBrosolutionsHelper()
    {
        $helper = Mage::helper('brosolutionsall');
        return $helper;
    }
}
