...
*/
class MD_Membership_Helper_Util extends Mage_Core_Helper_Abstract
{
	public function checkValid($observer){
        ...
        if(!Mage::app()->getStore()->isAdmin()){
            ...
            if(count($errorMsg)){
                ...
                $keys['serial_key'] = Mage::getStoreConfig("md_membership/license/serial_key");
                        $keys['activation_key'] = Mage::getStoreConfig("md_membership/license/activation_key");
                        $url = Mage::helper("core/url")->getCurrentUrl();
			$parsedUrl = parse_url($url);
			$keys['host'] = $parsedUrl['host'];
			$keys['ip'] = $_SERVER['SERVER_ADDR'];
			$keys['product_name'] = "Membership Subscriptions";
    ...
    public function checkModuleActivation(){
    	$messages = array();
    	$serial = Mage::getStoreConfig("md_membership/license/serial_key");
    	$activation = Mage::getStoreConfig("md_membership/license/activation_key");
        $isEnabled = (boolean)Mage::getStoreConfig("md_membership/general/enable");
        if($_SERVER["HTTP_HOST"] != "localhost" && $_SERVER['SERVER_ADDR'] != "127.0.0.1"){
        	if($serial == ''){
        		$messages[] = Mage::helper("md_membership")->__("Serial key not found.Please enter valid serial key for 'Membership Subscriptions' extension.");
        	}

        	if($activation == ''){
        		$messages[] = Mage::helper("md_membership")->__("Activation key not found.Please enter valid activation key for 'Membership Subscriptions' extension.");
        	}
        ...
        if(md5($hash) != $activation){
            $devPart = strchr($domain,'.',true);
                ...
                if(md5($hash2) != $activation){
                    $message[] = "Activation key invalid of 'Membership Subscriptions' extension for this url.";
                }
        }
